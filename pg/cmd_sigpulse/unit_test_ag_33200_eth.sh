...
stdbuf -oL -eL cmdmod /opt/pyrame/cmd_ag_33200.xml > ag_33200.trace 2>&1 &
...
sleep 1s
exec_n_test ./init.sh sigpulse_test 0 "ag_33200(bus=gpib(bus=tcp(host=10.220.0.71),dst_addr=10))" undef
id=`cat ent.trace | awk -F= '{ print $3 }'`
exec_n_test ./config.sh $id 4.4 2.02 82.3 3e-3 min min 2

exec_n_test ../cmd_ag_33200/get_frequency.sh 0
